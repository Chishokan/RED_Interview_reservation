...
> URLの1つ目の区切り(`/red`、`/chutobu`)が部門を表します。`vercel.json` の `rewrites` で
> `/<部門>` を `booking.html` に、`/<部門>/admin` を `admin.html` に割り当てています。
> 予備の経路として `booking.html?dept=red` のようなクエリ指定も受け付けます。
