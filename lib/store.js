 ...
 *
 * すべての入口で部門(dept)を受け取り、その部門の校舎かどうかを必ず確認する。
 * 他部門の枠や予約に触れられないようにするための要。
 *
 ...
 */
export async function getAvailableSlots(schoolId, dept) {
...
export async function getMyBookings(email, dept) {
  if (!email) return [];
  const db = getDb();
  // その部門の校舎に限定する(他部門の予約が混ざらないようにするため)
...
export async function createBooking(body) {
  const { schoolId, email, date, time, childName, parentName, grade, note, dept } = body || {};
...
export async function cancelBooking(body) {
  const { schoolId, email, id, dept } = body || {};
  const school = await findSchoolById(schoolId, dept);
  if (!school) return { ok: false, error: '校舎が見つかりません' };
...
export async function updateBooking(body) {
  const { schoolId, email, id, date, time, childName, parentName, grade, note, dept } = body || {};
  const school = await findSchoolById(schoolId, dept);
  if (!school) return { ok: false, error: '校舎が見つかりません' };
  ...
  if (formatDate(existing.date) !== newDate || formatTime(existing.time) !== newTime) {
    const available = await getAvailableSlots(schoolId, dept);
...
export async function getAllSlots(schoolId, dept) {
  const school = await findSchoolById(schoolId, dept);
  if (!school) return [];
  const db = getDb();
...
/** 校舎の全予約を返す */
export async function getAllBookings(schoolId, dept) {
  const school = await findSchoolById(schoolId, dept);
  if (!school) return [];
  const db = getDb();
  const rows = unwrap(
...
export async function adminAddSlots(body) {
  ...
  const school = await findSchoolById(schoolId, body.dept);
  if (!school) return { ok: false, error: '校舎が見つかりません' };
...
export async function adminDeleteSlot(body) {
  const { schoolId, id, dept } = body || {};
  const result = await adminDeleteSlots({ schoolId, dept, ids: [id] });
