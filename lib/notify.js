 ...
 */
async function sendStaffNotifications(booking) {
  const results = [];
  const dateLabel = formatJapaneseDate(booking.date);
...
/** 通知を待つ上限(ミリ秒)。環境変数 NOTIFY_TIMEOUT_MS で変更できる */
const DEFAULT_NOTIFY_TIMEOUT_MS = 7000;

function notifyTimeoutMs() {
  const v = Number(process.env.NOTIFY_TIMEOUT_MS);
  return Number.isFinite(v) && v > 0 ? v : DEFAULT_NOTIFY_TIMEOUT_MS;
}

/**
 * 担当者へ新規予約を通知する。
 *
 * 予約は既にDBに保存済みなので、通知の都合で保護者にエラーを見せてはいけない。
 * メール・LINE WORKS それぞれに個別のタイムアウトを入れてあるが、
 * 万一それも効かない場合に備えて全体の上限で必ず打ち切る。
 * 打ち切った送信が後から成立することはあり得るが、二重送信にはならない。
 */
export function notifyStaffNewBooking(booking) {
  const ms = notifyTimeoutMs();
  let timer;
  const capped = new Promise((resolve) => {
    timer = setTimeout(() => {
      console.error(`新規予約通知: ${ms}msを超えたため待機を打ち切りました`, booking && booking.id);
      resolve({ sent: false, reason: 'timeout', results: [] });
    }, ms);
  });
  const work = sendStaffNotifications(booking).catch((err) => {
    console.error('新規予約通知に失敗:', err);
    return { sent: false, reason: String(err), results: [] };
  });
