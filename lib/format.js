...
/**
 * 日時(予約日時・リマインド送信済・記録更新日時)を読みやすい文字列にする。
 *
 * DBのtimestamptzはISO 8601(UTC)で返ってくるので日本時間の表記に直す。
 * 移行スクリプトが扱うスプレッドシートのシリアル値やDateにも対応している。
 */
export function formatDateTime(val) {
  ...
  if (typeof val === 'number' && Number.isFinite(val)) {
    ...
    // 小数部が無い(= 時刻を持たない)値は日付だけを返す
    const frac = val - Math.floor(val);
    return frac === 0 ? date : date + ' ' + formatTime(val);
  }
  const s = String(val).trim();
  // DBのtimestamptzは ISO 8601 で返ってくるので、日本時間の表記に直す
  if (/^\d{4}-\d{2}-\d{2}T/.test(s)) {
    const d = new Date(s);
    if (!Number.isNaN(d.getTime())) return formatInTimeZone(d);
  }
  return s;
}
...
/** Date を指定タイムゾーンの 'YYYY-MM-DD HH:MM' 表記にする */
export function formatInTimeZone(date, timeZone = TIMEZONE) {
  const dtf = new Intl.DateTimeFormat('en-CA', {
    timeZone,
    hourCycle: 'h23',
    year: 'numeric',
...
/** 指定タイムゾーンでの現在時刻を 'YYYY-MM-DD HH:MM' 文字列で返す */
export function nowStamp(timeZone = TIMEZONE, base = new Date()) {
  return formatInTimeZone(base, timeZone);
}
